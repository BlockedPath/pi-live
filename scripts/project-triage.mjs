...
async function getProject() {
  const res = await gql(
    ...
    { login: process.env.PROJECT_LOGIN, number: Number(process.env.PROJECT_NUMBER) }
  );
...
async function addItem(projectId, issueNodeId) {
  ...
  if (res.errors) throw new Error(`addProjectV2ItemById failed: ${JSON.stringify(res.errors)}`);
...
async function setPriority(projectId, itemId, fieldId, optionId) {
  ...
  if (res.errors) throw new Error(`updateProjectV2ItemFieldValue failed: ${JSON.stringify(res.errors)}`);
}
...
async function main() {
  ...
  if (!priorityField) throw new Error('Project has no single-select "Priority" field');
